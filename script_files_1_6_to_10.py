...
## Read Qrels
_qrels = pd.read_csv(args.qrels, sep=' ', names=['topic.id', 'iteration', 'docno', 'usefulness', 'doc.answer', 'credibility'])
...
assert not (usefulness.eq('not.useful') & (doc_answer.eq('yes') | doc_answer.eq('no') | doc_answer.eq('NA'))).any()
# This assert fails on the old misinfo qrels
...
## File 6
qrels_binary_useful_correct = ((usefulness == 'useful') & (correct == 'TRUE')).map({True: 1, False: 0}).rename('useful.correct')
qrels_binary_useful_correct = pd.concat([qrels[["topic.id", "docno"]], qrels_binary_useful_correct], 1)
qrels_binary_useful_correct.insert(1, "iteration", 0)

qrels_binary_useful_correct.to_csv(output_dir / "misinfo-qrels-binary.useful-correct", index=False, sep=' ', header=False)
...
df = qrels.groupby('topic.id')['topic.id'].count().rename('total').to_frame().merge(df, left_index=True, right_index=True)
